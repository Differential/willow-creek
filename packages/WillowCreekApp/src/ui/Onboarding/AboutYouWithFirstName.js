...
import { AboutYouConnected, AboutYou, Slide, SlideContent } from '@apollosproject/ui-onboarding';
...
const AboutYouOverride = memo(
  ...
  <Slide {...props}>
    {BackgroundComponent}
    <SlideContent title={`Hello ${firstName}!`} description={description} icon>
...
const AboutYouWithFirstName = (props) => {
 return (
      ...
      {({ data }) => (
         <AboutYouOverride firstName={get(data, 'currentUser.profile.firstName', 'friend')} {...props}/>
      )}
    </Query>
   )
}
...
export default (props) => <AboutYouConnected {...props} Component={AboutYouWithFirstName} />